# Documentation and build-artefact tools (`packDocs`, `buildInfo`)

Merged from the experimental CMSIS Pack Docs extension. Two pure cores, two
handlers, one dispatch seam, wired into the MCP server and the multi-window
router the same way as the serial tool group.

## Layout

| Path | What | Imports `vscode`? |
| ---- | ---- | ----------------- |
| `src/core/packDocs/` | Target resolution from `*.cbuild-run.yml` (`cbuildRun.ts`, `targetDocs.ts`), pdsc `<book>` walking (`pdscBooks.ts`), Arm document catalogue and download (`armDocs.ts`, `webFetch.ts`), user and workspace document folders, PDF extraction — bundled pdf.js on a worker thread, `pdftotext` optional (`pdfExtract.ts`, `pdfWorker.ts`), the on-disk page store and BM25 index (`pageStore.ts`, `bm25Index.ts`, `search.ts`), peripheral dossiers over the SVD (`svdLite.ts`, `coreSvd.ts`, `peripheralDocs.ts`), renderers | no — everything arrives through `PackDocsHost` (`host.ts`) |
| `src/core/buildInfo/` | ELF32 reader (`elf.ts`, positioned reads, no library), GNU ld / armlink map parser (`mapFile.ts`), build-log diagnostics (`buildLog.ts`), artefact discovery from cbuild-run / cbuild files (`artifacts.ts`), region usage (`usage.ts`), renderers | no — `BuildInfoHost` |
| `src/packDocsHandler.ts`, `src/buildInfoHandler.ts` | One `handle*` method per tool, panel-only inspection methods; every call goes through the shared timeout fence and trace in `src/core/toolRun.ts` (`timeoutMs` up to 600 s — indexing a 3 000-page manual takes minutes; a call that outlives its timeout stays observed) | no |
| `src/packDocsTools.ts`, `src/buildInfoTools.ts` | The MCP tool names and zod schemas; call the dispatch, never a handler | no |
| `src/packDocsDispatch.ts` | `PackDocsHandlers { docs, build }`, `PackDocsDispatch`, `localPackDocsDispatch` | no |
| `src/packDocsHost.ts` | Settings (`cmsis-developer-assistant.packDocs.*`, `.buildInfo.*`), the host objects (pack root, `globalStorage/packdocs`, `assets/`, `findFiles`), `createPackDocsHandlers` | yes |
| `src/packDocsCommands.ts` | Palette commands: list / index target documentation, import a document for the current target (attribution wizard), open the user documents folder, open the panel | yes |
| `src/packDocsPanel.ts` | The Pack Docs webview panel: target and documents, SVD peripherals, page store (with the two confirmed clear actions: `PageStore.clearExtracted()` — sidecars only, downloads stay — and `clearDownloads()` — the `arm/` and `web/` trees; `storeUsage()` sizes the confirmation), in-process tool runner | yes |
| `assets/svd/core/` | 16 Cortex-M core-peripheral SVDs + `index.json`, generated by `scripts/core-svd/` (not shipped) | — |

## Wiring

- **Op table** (`src/core/opTable.ts`): `PACKDOCS_DOC_OPS` (5) and `PACKDOCS_BUILD_OPS` (5) form `PACKDOCS_OPS`; `isPackDocsOp` / `isPackDocsDocOp` pick the handler; the five documentation ops are `SLOW_OPS` (10-minute router floor). Compile-time coverage against the `handle*` methods of both handler classes.
- **Every window** builds the handler pair at activation (`extension.ts` → `createPackDocsHandlers`) — they do nothing until called — and hands it to its `ControlServer`, so a forwarded op executes in the window that owns the workspace: `dispatch(op)` is three-way (serial → serial singleton, pack-docs → `docs` or `build`, else the debugging handler).
- **Router**: `RoutingDebuggingHandler.packDocsOp` forwards like `serialOp`; `WindowCoordinator.sessionHandlers()` returns `packDocs: (op, args) => router.packDocsOp(op, args)`. The tools carry no path, so they resolve through the pinned / cached / sole-window rungs, exactly like `cmsis_action`.
- **Gates**: `DebugMCPServerOptions.packDocsEnabled` and `buildInfoEnabled` (default `false`), fixed per server instance; `setupTools` registers the two groups only when a gate is on and the session has a dispatch. The MCP `instructions` gain one sentence per enabled group. The bare single-window `DebugMCPServer` default (tests) has no dispatch and registers neither group.

## Extraction and ranking

- **Extractor** (`pdfExtract.ts`): `PdfjsExtractor` is the bundled default (`packDocs.extractor: auto`/`pdfjs`) — pdf.js legacy build, loaded on first use with a dynamic import; lines are rebuilt from text items by baseline and wide gaps become double spaces so table columns survive. The library runs on a `worker_threads` worker of its own (`pdfWorker.ts`, bundled by esbuild as `dist/pdfWorker.js` beside `extension.js`): pdf.js refuses to start when `Array.prototype` or `Object.prototype` has an enumerable property, and the extension host is shared with extensions that patch them (the CMSIS csolution extension assigns `Array.prototype.groupedBy`) — a worker thread is a fresh realm. `PdfjsExtractor` posts a file path and gets the page texts back; the thread is kept for the next document, retired after a minute idle, and terminated when an extraction times out. Inside the thread pdf.js does not detect Node (Electron), so `GlobalWorkerOptions.workerSrc` points at the shipped `node_modules/pdfjs-dist/legacy/build/pdf.worker.min.mjs` (re-included by `.vscodeignore`), imported there as a fake worker. `itemsToPageText` lives in `pdfText.ts`, shared by the worker and the tests. `PdftotextExtractor` (poppler) stays selectable; a document extracted by the other extractor is re-extracted on its next use so text sources never mix.
- **Tokenizer** (`tokenizer.ts`): NFKC before tokenising, identifier expansion (`RCC_AHB2ENR1` → `rcc`, `ahb2enr1`), hex spellings folded.
- **Index** (`bm25Index.ts`, version 2): body postings plus heading postings per page; BM25 over the body plus `headingWeight` (5) × the heading term score; a version-1 index on disk is rebuilt from `pages.jsonl` on first load. `search.ts` adds phrase, manual and table-of-contents adjustments and a 1.5 heading tie-breaker.
- **Query expansion** (`queryExpansion.ts`): identifier-only queries are expanded from the device SVD at half weight — a peripheral instance with its type synonyms and description, a bare field name with its register and description. Register names, prose and quoted phrases are not expanded (measured: it only dilutes). Expansions never gate the all-terms boost.
- **Benchmark** (`scripts/search-benchmark.ts`, `npm run bench:search`): gold pages are the manual headings that name a register, queries come from the SVD descriptions; reports R@1 / R@3 / MRR per heading weight and post boost, and can extract a PDF with either extractor first. Numbers in `docs/improvement-notes.md` §11.

## Storage

Extracted page text, metadata and indexes live under `<globalStorageUri>/packdocs/` — `<vendor>/<pack>/<version>/<slug>.{pages.jsonl,meta.json,idx.json}` for pack documents, `workspace/<hash>/…`, `user/<hash>/…`, `web|arm/<id>/…` for fetched ones. User-document ids carry their scope folder (`user/keil/stm32u5xx-dfp/rm0456`; `user/<name>` at the root), so a file name repeated in two scopes never collides. PDFs themselves stay where they are. The user documents folder defaults to `~/.cmsis-pack-docs/user` so documents imported with the standalone extension stay attributed.

## Tests

`src/test/{cbuildRun,pdscBooks,pageStore,search,svdLite,coreSvd,coreHeader,npuHeader,armDocs,webFetch,userDocs,workspaceDocs,chapters,headings,tokenizer,xmlLite,peripheralDocs,packDocsHandler,buildInfo*}.test.ts` with fixtures under `src/test/fixtures/{packdocs,buildinfo}` (trimmed pdsc files, a synthetic PDF, two small ELF/map pairs, two build logs). `pdftotext`-dependent tests skip when poppler is absent. `src/test/routing.test.ts` covers the op table and a control-server round trip to the right handler; `test/transport/session-lifecycle.js` measures the all-on tool list against its own budget and exercises the no-build answers; `test/transport/packaged-vsix.js` checks the SVDs and the skill ship.

## Follow-ups

Fold `svdLite.ts` (interrupts, `groupName`, flattened clusters) into `src/core/svdParser.ts`; use `core/packDocs/cbuildRun.ts` for the SVD and flash cbuild-run resolution too; drop `core/packDocs/textBudget.ts` in favour of `core/textBudget.ts` (the two have diverged in their clip suffix); turn the gates on by default once the tool-list cost is accepted.
